...
namespace TestMapPiloteGeoPackageHandler
{
    ...
    public class TestGeopackageInfo
    {
        ...
        public void Cleanup()
        {
            try
            {
                if (File.Exists(_gpkgPath)) File.Delete(_gpkgPath);
            }
            catch { /* ignore */ }
        }
        ...
        public void CreateFourLayers_And_ReadGeopackageInfo_ShouldMatch()
        {
            ...
            // Act
            var info = CMPGeopackageReadDataHelper.GetGeopackageInfo(_gpkgPath);

            // Assert: 4 layers present
            Assert.AreEqual(4, info.Layers.Count, "Expected exactly 4 user layers in gpkg_contents.");

            var layerNames = info.Layers.Select(l => l.TableName).OrderBy(n => n).ToArray();
            ...
            var srsIds = info.SpatialRefSystems.Select(s => s.SrsId).ToHashSet();
            ...
            {
                var layer = info.Layers.Single(l => l.TableName == name);
